  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          <p className="text-[11px] uppercase tracking-widest text-[#9a96b0]">Stock Agents</p>
          <h1 className="text-3xl font-semibold tracking-tight">Saved rules. Not autonomous traders.</h1>
        </div>
        <Link href="/terminal" className="px-4 py-2 bg-[#14f195] text-black text-sm font-medium">
          New strategy
        </Link>
      </div>

      {!wallet && <p className="text-[#9a96b0] text-sm">Connect a wallet to see agents bound to it.</p>}

      <div className="grid gap-3">
        {list.length === 0 && wallet && (
          <div className="panel p-8 text-[#9a96b0] text-sm">No agents yet.</div>
        )}
        {list.map((s) => (
          <div key={s.id} className="panel p-5 flex flex-wrap items-center justify-between gap-4">
                ...
                <span
                  className={cn(
                    ...
                    s.status === "ACTIVE"
                      ...
                      : s.status === "TRIGGERED"
                      ? "text-[#14f195] border-[#14f195]/40"
                      : "text-[#9a96b0] border-white/15"
                  )}
                >
                  {s.status}
                </span>
              </div>
              <div className="text-sm text-[#9a96b0] mt-1">
            ...
            <div className="flex gap-2 text-sm">
              ...
              <button
                ...
                className="px-3 py-1.5 panel text-[#ff6b8a]"
              >
                Delete
              </button>
            </div>
